...
public class ContainerPlayer implements TransportControl, SourcedTimerListener, TimeSource {
	...
	private ControllerListener controllerListener = new ControllerListener() {
		public void controllerUpdate(ControllerEvent event) {
			...
			{
				notifyStatusListener(ContainerPlayerStatusListener.ERROR_PREFIX + ((ControllerErrorEvent) event).getMessage());
			}
			...
			// if (isOperational())
			{
				...
				{
		  			if (transportControlListener != null)
		  				transportControlListener.onDurationChange(player.getDuration().getNanoseconds());
					if (transportControlListener != null)
				...
				{
		  			if (transportControlListener != null)
		  				transportControlListener.onDurationChange(player.getDuration().getNanoseconds());	// just in case
