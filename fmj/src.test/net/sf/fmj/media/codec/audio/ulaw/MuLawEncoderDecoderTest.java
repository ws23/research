...
public class MuLawEncoderDecoderTest extends TestCase 
{
	...
	public void testTableLookup()
	{
		for (int i = 0; i <= 255; ++i)
		{
			...
			short sLookup = (short) ULAW_TO_LINEAR_16_BIT[UnsignedUtils.uByteToInt(b)];
	...
	public void testShorts()
	{
		for (int i = Short.MIN_VALUE; i <= Short.MAX_VALUE; ++i)
		{
			...
			for (int j = 0; j < ULAW_TO_LINEAR_16_BIT.length - 1; ++j)
			{
				if (s < 0)
				{
					if (s > ULAW_TO_LINEAR_16_BIT[j] && s < ULAW_TO_LINEAR_16_BIT[j + 1])
					{
						final boolean eq0 = bEncoded == (byte) j;
						final boolean eq1 = bEncoded == (byte) (j + 1);
						if (!eq0 && !eq1)
							System.err.println("s=" + s + " j=" + j + " bEncoded=" + bEncoded + " ULAW_TO_LINEAR_16_BIT[j]=" + ULAW_TO_LINEAR_16_BIT[j] + " ULAW_TO_LINEAR_16_BIT[j + 1]=" + ULAW_TO_LINEAR_16_BIT[j + 1]);
						assertTrue(eq0 || eq1);
					}
				}
				else if (s > 0)
	...
	private void test(byte b)
	{
		...
		if (b == 127)
		{	assertEquals(bEncoded, -1);	// there are 2 values that can yield the same short value of 0 - -1 and 127.
			assertEquals(s, 0);
		}
		else	
