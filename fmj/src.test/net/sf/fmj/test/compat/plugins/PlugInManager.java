...
{
	// TODO: what exactly is stored in the properties?  just the class names, or the formats as well?
	// the properties files appears to be binary, an appears to be created using java serialization.
	// seems like it contains the formats.
	// TODO: implement efficiently using maps
	// Vectors of PlugInInfo.
	private static final Vector[] vectors = new Vector[] {new Vector(), new Vector(), new Vector(), new Vector(), new Vector()};
	private static final Vector[] filteredVectors = new Vector[] {null, null, null, null, null};
	...
	public static synchronized boolean addPlugIn(String classname, Format[] in, Format[] out, int type)
	{
		...
		{
			System.err.println("Unable to addPlugIn for " + classname + " due to inability to get its class: " + t);
			t.printStackTrace();
			return false;
		}
