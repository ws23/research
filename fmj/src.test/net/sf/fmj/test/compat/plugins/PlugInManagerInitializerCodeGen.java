...
public class PlugInManagerInitializerCodeGen
{
	...
	public void run()
	{
		for (int i = 1; i <= 5; ++i)
		{
			...
			for (int j = 0; j < v.size(); ++j)
			{
				...
				System.out.print(s);
				System.out.print("\", ");
				final Format[] in = PlugInManager.getSupportedInputFormats(s, i);
				dumpFormatArray(in);
				
				System.out.print(", ");
				final Format[] out = PlugInManager.getSupportedOutputFormats(s, i);
				dumpFormatArray(out);
				System.out.println(", ");
	...
	private static String typeToStr(int type)
	{
		switch (type)
		{
			case PlugInManager.DEMULTIPLEXER: return "PlugInManager.DEMULTIPLEXER";
	
			case PlugInManager.CODEC: return "PlugInManager.CODEC";
	
			case PlugInManager.EFFECT: return "PlugInManager.EFFECT";
	
			case PlugInManager.RENDERER: return "PlugInManager.RENDERER";
	
			case PlugInManager.MULTIPLEXER: return "PlugInManager.MULTIPLEXER";
