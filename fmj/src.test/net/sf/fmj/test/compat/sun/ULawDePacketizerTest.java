...
public class ULawDePacketizerTest extends TestCase
{
	...
	public void testDePacketizer()
	{
		...
		assertEquals(f[0], new AudioFormat("ULAW/rtp", -1.0, -1, -1, -1, -1, -1, -1.0, Format.byteArray));
		

		Format[] f2 = p.getSupportedOutputFormats(f[0]);
		assertEquals(f2.length, 1);
		assertEquals(f2[0], new AudioFormat("ULAW", -1.0, -1, -1, -1, -1, -1, -1.0, Format.byteArray));
		...
		Format[] f5 = p.getSupportedOutputFormats(new AudioFormat(AudioFormat.DOLBYAC3));
